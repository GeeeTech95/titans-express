...
class Station(models.Model):
    ...
    country = models.ForeignKey(
        Country, related_name='stations', on_delete=models.CASCADE)
...
class Shipment(models.Model):
    ...
    tracking_number = models.CharField(
        default=get_tracking_number, max_length=8, blank=True)
    is_fragile = models.BooleanField(default=False)
    is_paid = models.BooleanField(default=False, blank=True)
    shipment_fee = models.DecimalField(
        decimal_places=2, max_digits=100, help_text="in dollars($)")

    # shipment info
    source_country = models.ForeignKey(
        Country, on_delete=models.CASCADE, related_name="shipment_source")
    source_address = models.CharField(max_length=200)
    destination_country = models.ForeignKey(
        Country, on_delete=models.CASCADE, related_name="shipment_destination")
    ...
    def update_status_log(self, date=None):
        ...
        if _index < len(statuses) - 1:
            ...
            StatusLog.objects.create(
                shipment=self, status=statuses[_index + 1], date=date)
        else:
            # meaning error
            return "You cannot update status log any further, shipment is already in {}".format(current_status)
    ...
    def roll_back_status_log(self):
        ...
        if _index > 0:
            ...
            # meaning error
            return "You cannot roll back status log any further, shipment is in {} status".format(current_status)
    ...
    def timeline_logs(self):
        ...
        try:
            transit_logs = self.transit_logs.order_by("date")
...
class StatusLog(models.Model):
    ...
    shipment = models.ForeignKey(
        Shipment, related_name="status_logs", on_delete=models.CASCADE)
...
class TransitLog(models.Model):
    ...
    shipment = models.ForeignKey(
        Shipment, related_name='transit_logs', on_delete=models.CASCADE)
    status = models.CharField(max_length=10, choices=StatusChoices)
...
class Invoice(models.Model):
    def get_invoice_id():
        return "TINV{}".format(''.join(filter(str.isdigit, str(uuid.uuid4()))).upper()[:6])

    invoice_number = models.CharField(max_length=10, default=get_invoice_id)
    # if a shipment already has an invoice the invoicee is deleted in favour of a new invoice
